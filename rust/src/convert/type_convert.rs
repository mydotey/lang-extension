...
pub trait TypeConverter<S: ?Sized + ValueConstraint, T: ?Sized + ValueConstraint>:
    RawTypeConverter
{
...
macro_rules! raw_type_converter {
    ($source_type: ty, $target_type: ty) => {
        ...
        fn convert_raw(&self, source: &dyn Value) -> Result<Box<dyn Value>, Box<dyn Value>> {
            match source.as_any_ref().downcast_ref::<$source_type>() {
                Some(s) => match $crate::convert::TypeConverter::convert(self, s) {
                    Ok(t) => Ok(Value::to_boxed(*t)),
                    Err(err) => Err(err),
                },
                None => Err(Box::new(format!(
                    "source value {:?} is of an unsupported type: {:?}, only support: {:?}",
                    source,
                    source.type_name(),
    ...
    ($type: tt<$source_type:tt, $target_type:tt>) => {
        ...
        unsafe impl<$source_type: ?Sized + ValueConstraint, $target_type: ?Sized + ValueConstraint>
            Sync for $type<$source_type, $target_type>
        {
        }
        unsafe impl<$source_type: ?Sized + ValueConstraint, $target_type: ?Sized + ValueConstraint>
...
impl<S: ?Sized + ValueConstraint, T: ?Sized + ValueConstraint> PartialEq
    for DefaultTypeConverter<S, T>
{
    fn eq(&self, other: &Self) -> bool {
        self.convert
            .as_ref()
            .reference_equals(other.convert.as_ref())
    }
}

impl<S: ?Sized + ValueConstraint, T: ?Sized + ValueConstraint> Eq for DefaultTypeConverter<S, T> {}

impl<S: ?Sized + ValueConstraint, T: ?Sized + ValueConstraint> fmt::Debug
    for DefaultTypeConverter<S, T>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "{} {{ source_type: {}, target_type: {}, converter: {} }}",
...
pub fn new_inplace_type_converter<V: ?Sized + ValueConstraint>() -> Box<InplaceTypeConverter<V>> {
    TypeConverter::to_boxed(DefaultTypeConverter::<V, V>::new(Box::new(|v| {
        Ok(Box::new(v.clone()))
    })))
}
    ...
    fn type_convert() {
        ...
        match C.convert_raw(Value::as_trait_ref(&9)) {
            Ok(s) => {
                ...
                assert_eq!(
                    "9".to_string(),
                    *s.as_ref().as_any_ref().downcast_ref::<String>().unwrap()
                );
            }
            Err(err) => {
                println!("{:?}", err);
